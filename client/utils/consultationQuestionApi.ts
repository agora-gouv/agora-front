...
import type { ConsultationQuestionsApiDTO } from "~/types/consultation/consultationQuestionsApiDTO";
import type { ConsultationAnswersApiDTO } from "~/types/consultation/consultationAnswersApiDTO";
...
export class ConsultationQuestionApi {
  ...
  async getQuestions(consultationId: string) {
    ...
    const { data: questions, error: errorQuestions } = (await useFetch(routeQuestionsUrl)) as AsyncData<
  ...
  async sendAnswers(
    ...
    answersCheckbox: Record<string, string | string[]>,
    ...
    const dtoAnswers = Object.entries(answersCheckbox)
      ...
      .filter((response) => response.choiceIds.length > 0 || response.responseText !== "");
    ...
    const { data: questions, error: errorQuestions } = (await useFetch(routeConsultationUrl, {
      method: "POST",
