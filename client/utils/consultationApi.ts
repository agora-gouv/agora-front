...
import type { ConsultationResultsApiDTO } from "~/types/consultation/consultationResultsApiDTO";
import type ConsultationsListApiDTO from "~/types/consultation/consultationsList";
...
export class ConsultationApi {
  ...
  async getConsultation(consultationId: string, jwtToken: string | null) {
    ...
    const { data: consultation, error: errorConsultation } = (await useFetch(routeConsultationUrl, jwtOption)) as AsyncData<
      Consultation,
      FetchError
    >;
  ...
  async getConsultationUpdate(consultationId: string, consultationUpdateId: string, jwtToken: string | null) {
    ...
    const { data: consultationUpdate, error } = (await useFetch(routeUrl, jwtOption)) as AsyncData<Consultation, FetchError>;
  ...
  async getConsultationResults(consultationId: string) {
    ...
    const { data: consultationResults, error: errorConsultation } = (await useFetch(routeConsultationResponsesUrl)) as AsyncData<
  ...
  async getAllConsultations() {
    ...
    const { data: consultations, error: errorConsultation } = (await useFetch(routeConsultationResponsesUrl)) as AsyncData<
      ConsultationsListApiDTO,
      FetchError
    >;
